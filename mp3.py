...
pause_btn = Button(button_frame, text='Pause', bg='Aqua', font=("Georgia", 13), width=7, command=lambda: pauza(song_status))
pause_btn.place(x=15, y=10)

stop_btn = Button(button_frame, text='Stop', bg='Aqua', font=("Georgia", 13), width=7, command=lambda: zaustavi(song_status))
stop_btn.place(x=100, y=10)

play_btn = Button(button_frame, text='Play', bg='Aqua', font=("Georgia", 13), width=7, command=lambda: pusti(ime_pesme, playlist, song_status))
play_btn.place(x=185, y=10)

resume_btn = Button(button_frame, text='Resume', bg='Aqua', font=("Georgia", 13), width=7, command=lambda: resume(song_status))
resume_btn.place(x=270, y=10)

skip_btn = Button(button_frame, text='Next', bg='Aqua', font=("Georgia", 13), width=10, command=lambda: preskoci(song_status, playlist))
skip_btn.place(x=355, y=10)

previous_btn = Button(button_frame, text='Previous', bg='Aqua', font=("Georgia", 13), width=10, command=lambda: prethodna(song_status, playlist))
previous_btn.place(x=470, y=10)

shuffle_btn = Button(button_frame, text='Shuffle', bg='Aqua', font=("Georgia", 13), width=10, command=lambda: shuffle(ime_pesme, playlist, song_status))
